...
class ActivateSimStepIdentity extends Component {
  constructor(props) {
    super(props);
  ...
  handleNext() {
    ...
    this.validator.validateAll(this.validationData).then(
      (success) => {
        if (success) {
          this.props.history.push(UrlHelper.getMainUrl('activate-sim/step-plan'));
        } else {
          this.setState({
            errors: this.validator.errors
          });
    ...
    return (
      <div className={'section'}>
        <div className={'container'}>
          {this.renderModal()}
          <Step steps={['Activation Code', 'Information', 'Address', 'Identity', 'Select plan', 'Confirm']} index={3} />
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(ActivateSimStepIdentity));
